...
class HomeCubit extends Cubit<HomeState> {
  ...
  Future<void> fetchCurrentList() async {
    ...
    final result = await homeRepository.getObjectsIdQuery(query: "Current%20Exhibitions");
    if (result is SuccessDataResult<ObjectsIdModel> && result.data != null) {
      final objectsIdModel = result.data!;
      final ids = objectsIdModel.objectIDs.take(4).toList();
      ...
      emit(state.copyWith(
        isLoading: false,
        currentObjectsIdModel: objectsIdModel,
        currentList: details,
        errorMessage: null,
      ));
    } else {
  ...
  Future<void> fetchfamousArtList() async {
    emit(state.copyWith(isLoading: true, errorMessage: null));
    final result = await homeRepository.getObjectsIdQuery(query: "Famous%20Artworks");
    if (result is SuccessDataResult<ObjectsIdModel> && result.data != null) {
      final objectsIdModel = result.data!;
      final ids = objectsIdModel.objectIDs.take(4).toList();
      List<ObjectModel> details = [];
      ...
      emit(state.copyWith(
        isLoading: false,
        famousObjectsIdModel: objectsIdModel,
        famousArtworkList: details,
        errorMessage: null,
      ));
    } else {
      emit(state.copyWith(
  ...
  Future<void> fetchObjectDetails(List<int> objectIds) async {
    emit(state.copyWith(isLoading: true, errorMessage: null));
    List<ObjectModel> details = [];
    for (final id in objectIds) {
      final result = await homeRepository.getObjectDetails(objectId: id);
      if (result is SuccessDataResult<ObjectModel> && result.data != null) {
        details.add(result.data!);
      }
    }
    emit(state.copyWith(isLoading: false, currentList: details));
  }
